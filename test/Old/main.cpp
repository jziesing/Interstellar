				...
				for(int j=0; j<simulator.num_UFO; j++){
					...
					for(int i=0; i<800; i++){
						...
						*/
						...
						/*
						*this block controls the CANNONS
						*
						*We'll want to do this in some kind of loop when a randomized number of cannons is used
						*/
						//cannon 1
						Sprite * CANNON_sprite1 = (simulator.CANNON_arr[0])->getSprite();
						CANNON_sprite1->setPosition(50, 500);
						window.draw(*CANNON_sprite1);
						//cannon 2
						Sprite * CANNON_sprite2 = (simulator.CANNON_arr[1])->getSprite();
						CANNON_sprite2->setPosition(300, 500);
						//CANNON_sprite2->setOrigin(300, 500);
						CANNON_sprite2->rotate(i);
						window.draw(*CANNON_sprite2);
						//cannon 3
						Sprite * CANNON_sprite3 = (simulator.CANNON_arr[2])->getSprite();
						CANNON_sprite3->setPosition(600, 500);
						window.draw(*CANNON_sprite3);
